// src/app/pages/admin/roomAnalytics.ts
...
export function buildUtilizationByRoomName(
  ...
  const includeStatuses = opts?.includeStatuses ?? ["completed", "confirmed", "active", "upcoming"];
  ...
  return scopedResources
    .map((r) => {
      ...
      const util = totalRangeMin <= 0 ? 0 : clamp((bookedMin / totalRangeMin) * 100, 0, 100);
      ...
      return {
        ...
        availableMinutes: Math.round(totalRangeMin),
      };
    })
...
export function buildRoomTimebandHeatmap(
  ...
  const availMinPerBand = bands.map(() => 0);
  {
    ...
    for (let d = new Date(dayStart); d < rangeEnd; d.setDate(d.getDate() + 1)) {
      ...
      for (let i = 0; i < bands.length; i++) {
        ...
        if (e > s) availMinPerBand[i] += (e.getTime() - s.getTime()) / MS_MIN;
      }
    }
  }
    ...
    rooms: scopedResources.map((r) => {
      ...
      return {
        ...
        cells: bookedArr.map((bm, i) => {
          const denom = availMinPerBand[i];
          const pct = denom <= 0 ? 0 : clamp((bm / denom) * 100, 0, 100);
